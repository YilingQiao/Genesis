...
def mpr_discover_portal(
    ...
    if (ti.abs(normal_ws) < CCD_EPS).all():
        center_a = gu.ti_transform_by_trans_quat(geoms_info_center[i_ga], geom_state_pos[i_ga], geom_state_quat[i_ga])
        center_b = gu.ti_transform_by_trans_quat(geoms_info_center[i_gb], geom_state_pos[i_gb], geom_state_quat[i_gb])
        ...
        center_a = gu.ti_transform_by_trans_quat(center_a_local, geom_state_pos[i_ga], geom_state_quat[i_ga])
        center_b_local = 0.5 * (geoms_init_AABB[i_gb, 7] + geoms_init_AABB[i_gb, 0])
        center_b = gu.ti_transform_by_trans_quat(center_b_local, geom_state_pos[i_gb], geom_state_quat[i_gb])
        delta = center_a - center_b
        ...
        if (ti.abs(normal_ws) > CCD_EPS).any() or normal_ws.cross(normal).norm() > CCD_TOLERANCE:
            ...
            if offset_norm > CCD_TOLERANCE:
                ...
                # First, move the direction in local box frame
                dir_offset = offset / offset_norm
                dir_offset_local_a = gu.ti_transform_by_quat(dir_offset, gu.ti_inv_quat(geom_state_quat[i_ga]))
                dir_offset_local_b = gu.ti_transform_by_quat(dir_offset, gu.ti_inv_quat(geom_state_quat[i_gb]))
...
def mpr_find_penetr_touch(
    ...
    is_col = True
    penetration = gs.ti_float(0.0)
    normal = -simplex_support_v[i_ga, i_gb, 0, i_b].v.normalized()
...
def mpr_find_penetr_segment(
    ...
    is_col = True
    penetration = simplex_support_v[i_ga, i_gb, 1, i_b].v.norm()
    normal = -simplex_support_v[i_ga, i_gb, 1, i_b].v.normalized()
    pos = (simplex_support_v1[i_ga, i_gb, 1, i_b] + simplex_support_v2[i_ga, i_gb, 1, i_b]) * 0.5
...
def mpr_portal_encapsules_origin(
    ...
    dot = simplex_support_v[i_ga, i_gb, 1, i_b].v.dot(direction)
    return dot > -CCD_EPS
...
def mpr_portal_reach_tolerance(
    ...
    dv1 = simplex_support_v[i_ga, i_gb, 1, i_b].v.dot(direction)
    dv2 = simplex_support_v[i_ga, i_gb, 2, i_b].v.dot(direction)
    dv3 = simplex_support_v[i_ga, i_gb, 3, i_b].v.dot(direction)
    dv4 = v.dot(direction)
    dot1 = ti.min(dv4 - dv1, dv4 - dv2, dv4 - dv3)
...
def mpr_expand_portal(
    ...
    v4v0 = v.cross(simplex_support_v[i_ga, i_gb, 0, i_b].v)
    dot = simplex_support_v[i_ga, i_gb, 1, i_b].v.dot(v4v0)

    if dot > 0:
        dot = simplex_support_v[i_ga, i_gb, 2, i_b].v.dot(v4v0)
        if dot > 0:
            simplex_support_v1[i_ga, i_gb, 1, i_b] = v1
            simplex_support_v2[i_ga, i_gb, 1, i_b] = v2
            simplex_support_v[i_ga, i_gb, 1, i_b] = v
        ...
        dot = simplex_support_v[i_ga, i_gb, 3, i_b].v.dot(v4v0)
        if dot > 0:
            simplex_support_v1[i_ga, i_gb, 2, i_b] = v1
            simplex_support_v2[i_ga, i_gb, 2, i_b] = v2
            simplex_support_v[i_ga, i_gb, 2, i_b] = v
...
def mpr_find_pos(
    ...
    if sum_ < CCD_EPS:
        ...
        for i in range(1, 4):
            i1, i2 = i % 3 + 1, (i + 1) % 3 + 1
            vec = simplex_support_v[i_ga, i_gb, i1, i_b].v.cross(simplex_support_v[i_ga, i_gb, i2, i_b])
            b[i] = vec.dot(direction)
        sum_ = b.sum()
